...
def load_earthquakes():
    ...
    if CSV_PATH.exists():
        ...
        # Fallback for deploys where local CSV is unavailable.
        df = fetch_usgs_fallback_data()
...
def fetch_usgs_fallback_data(limit: int = 1000) -> pd.DataFrame:
    # Fetch recent USGS events to keep endpoints functional without local data file.
    response = requests.get(
        USGS_API_URL,
        params={"format": "csv", "limit": limit, "orderby": "time", "minmagnitude": 2.5},
        timeout=30,
    )
    ...
    if not csv_text:
        raise RuntimeError("USGS returned an empty CSV response.")
